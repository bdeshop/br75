...
// Sidebar.jsx receives two new props:
//   onExpandAndActivate(category) - called when sidebar is CLOSED and a category icon is clicked
//   externalActiveMenu            - controlled active menu from Header (string | null)
const Sidebar = ({ sidebarOpen, onCategorySelect, onExpandAndActivate, externalActiveMenu }) => {
  ...
  // Sync external active menu (set by Header when it opens the sidebar)
  useEffect(() => {
    if (externalActiveMenu !== undefined && externalActiveMenu !== null) {
  ...
  const handleCategoryItemClick = async (category) => {
    // If sidebar is closed: tell Header to open it and activate this category
    if (!sidebarOpen) {
      if (onExpandAndActivate) {
        onExpandAndActivate(category);
      }
      return;
    }
