...
const Pendingwithdraw = () => {
  ...
  const [stats, setStats] = useState({ total: 0, pending: 0, totalAmount: 0, completedAmount: 0 });
  ...
  const fetchWithdrawals = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/admin/withdrawals`, {
        ...
        params: {
          page: currentPage, limit: itemsPerPage, status: 'pending',
      ...
      if (response.data) {
        ...
        setStats({
          total: arr.length,
          pending: arr.filter((w) => w.status === 'pending').length,
      ...
      const sampleData = [
        { _id: '69c4c57a9763d121d14b47c0', userId: { username: 'testuser', player_id: 'PID123456' }, method: 'bkash', mobileBankingDetails: { phoneNumber: '01655585555', accountType: 'personal' }, bankDetails: null, amount: 500, status: 'pending', transactionId: null, processedAt: null, adminNote: null, createdAt: '2026-03-26T05:34:50.687Z' },
        { _id: '69c4c5be9763d121d14b4803', userId: { username: 'testuser', player_id: 'PID123456' }, method: 'bank', mobileBankingDetails: null, bankDetails: { bankName: 'Dutch Bangla Bank', accountHolderName: 'John Doe', accountNumber: '435345345345', branchName: 'Main Branch', district: 'Dhaka', routingNumber: '123456789' }, amount: 500, status: 'pending', transactionId: null, processedAt: null, adminNote: null, createdAt: '2026-03-26T05:35:58.042Z' },
      ];
      setWithdrawals(sampleData);
      setStats({ total: sampleData.length, pending: sampleData.length, totalAmount: 1000, completedAmount: 0 });
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    try {
      ...
      const csvData = withdrawals.map((w) => [formatDate(w.createdAt), w.userId?.player_id || 'N/A', w.userId?.username || 'N/A', getMethodName(w.method), w.amount, getAccountDetails(w).fullDetails, w.status, w.transactionId || 'N/A']);
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      ...
      case 'failed': case 'cancelled': return { icon: <FaTimesCircle className="text-rose-400" />, badge: 'bg-rose-500/10 text-rose-400 border border-rose-500/20' };
      ...
      <div className="flex pt-[10vh]">
        ...
        <main className={`transition-all duration-300 flex-1 p-6 overflow-y-auto h-[90vh] ${isSidebarOpen ? 'md:ml-[40%] lg:ml-[28%] xl:ml-[17%]' : 'ml-0'}`}>
          ...
          <div className="rounded-lg mb-8 flex flex-col md:flex-row justify-between items-center">
            <div>
              <h1 className="text-2xl font-semibold text-white tracking-tighter uppercase">Pending Withdrawals</h1>
              <p className="text-xs font-bold text-gray-500 mt-1 flex items-center gap-2">
                <FaCalendarAlt className="text-amber-500" /> Manage pending withdrawal transactions
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
            {[
              { label: 'TOTAL PENDING', value: stats.pending, color: 'border-amber-500', valueClass: 'text-amber-400' },
              { label: 'TOTAL AMOUNT', value: `৳${formatCurrency(stats.totalAmount)}`, color: 'border-rose-500', valueClass: 'text-rose-400' },
              { label: 'COMPLETED AMOUNT', value: `৳${formatCurrency(stats.completedAmount)}`, color: 'border-emerald-500', valueClass: 'text-emerald-400' },
            ].map((card, i) => (
          ...
          <div className="bg-[#161B22] border border-gray-800 rounded-lg p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setSearchTerm(''); setMethodFilter('all'); setDateRange({ start: '', end: '' }); }} className="text-[10px] text-amber-400 hover:text-amber-300 font-bold uppercase tracking-wider">
                Clear All
              </button>
            </div>
          ...
          <div className="mb-3">
            <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Showing {withdrawals.length} of {stats.total} pending withdrawals</p>
          </div>
                  ...
                  {loading ? (
                    <tr><td colSpan="7" className="px-6 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-[10px] uppercase tracking-widest font-bold text-gray-500">Loading pending withdrawals...</p>
                      </div>
                    </td></tr>
                    ...
                    return (
                      <tr key={w._id} className="hover:bg-[#1F2937] transition-colors">
                        <td className="px-5 py-4 text-xs text-gray-400 whitespace-nowrap">{formatDate(w.createdAt)}</td>
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <span className="text-sm font-black text-amber-400">৳{formatCurrency(w.amount)}</span>
                        </td>
                    ...
                    <tr><td colSpan="7" className="px-6 py-16 text-center">
                      <div className="flex flex-col items-center text-gray-600">
                        ...
                        <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">No pending withdrawals found</p>
          ...
          {totalPages > 1 && (
            <div className="mt-5 flex flex-col sm:flex-row justify-between items-center gap-3">
              ...
              <nav className="flex items-center gap-1">
                <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className={`px-3 py-1.5 rounded text-xs font-bold border transition-all ${currentPage === 1 ? 'bg-[#1C2128] border-gray-800 text-gray-700 cursor-not-allowed' : 'bg-[#1C2128] border-gray-700 text-gray-300 hover:bg-amber-600/30 hover:border-amber-500/50'}`}>← Prev</button>
                {getPaginationPages().map((page, idx) => page === '...' ? (
                  <span key={`e-${idx}`} className="px-2 py-1.5 text-xs text-gray-600 font-bold">···</span>
                ) : (
                  <button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-1.5 rounded text-xs font-bold border transition-all ${currentPage === page ? 'bg-amber-600 border-amber-500 text-white' : 'bg-[#1C2128] border-gray-700 text-gray-300 hover:bg-amber-600/30 hover:border-amber-500/50'}`}>{page}</button>
                ))}
                <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className={`px-3 py-1.5 rounded text-xs font-bold border transition-all ${currentPage === totalPages ? 'bg-[#1C2128] border-gray-800 text-gray-700 cursor-not-allowed' : 'bg-[#1C2128] border-gray-700 text-gray-300 hover:bg-amber-600/30 hover:border-amber-500/50'}`}>Next →</button>
              </nav>
            </div>
          )}
        </main>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[1000] backdrop-blur-sm p-4">
          <div className="bg-[#161B22] border border-gray-700 rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-800 flex justify-between items-center bg-[#1C2128]">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-amber-400">Pending Withdrawal Details</h3>
            ...
            <div className="px-6 py-4 border-t border-gray-800 bg-[#1C2128] flex justify-end gap-3">
              <button onClick={() => { closeWithdrawalDetails(); openUpdateModal(selectedWithdrawal); }} className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded text-xs font-bold transition-all">Update Status</button>
      ...
      {showDeleteModal && selectedWithdrawal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[10000] backdrop-blur-sm p-4">
          <div className="bg-[#161B22] border border-gray-700 rounded-lg shadow-2xl max-w-md w-full">
            ...
            <div className="px-6 py-5">
              <p className="text-xs text-gray-400 mb-5">Are you sure you want to delete this pending withdrawal request? This action cannot be undone.</p>
              <div className="bg-rose-500/5 border border-rose-500/20 rounded p-4">
                ...
                <div className="space-y-1.5">
                  {[['Amount', `৳${formatCurrency(selectedWithdrawal.amount)}`], ['User', `${selectedWithdrawal.userId?.username || 'Unknown'} (${selectedWithdrawal.userId?.player_id || 'N/A'})`], ['Method', getMethodName(selectedWithdrawal.method)], ['Account', getAccountDetails(selectedWithdrawal).fullDetails], ['Status', selectedWithdrawal.status]].map(([label, val]) => (
