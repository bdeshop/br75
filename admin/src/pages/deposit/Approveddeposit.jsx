...
import { FaSearch, FaEye, FaSort, FaSortUp, FaSortDown, FaCheckCircle, FaEdit, FaTrash, FaSpinner } from 'react-icons/fa';
...
const Approveddeposit = () => {
  ...
  const [stats, setStats] = useState({ total: 0, totalAmount: 0, approvedAmount: 0 });
  ...
  const deleteDeposit = async (depositId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_BASE_URL}/admin/deposits/${depositId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchDeposits();
      fetchStats();
  ...
  const getSortIcon = (key) => {
    ...
    if (sortConfig.direction === 'ascending') return <FaSortUp className="text-emerald-400 inline ml-1" />;
    return <FaSortDown className="text-emerald-400 inline ml-1" />;
  };
  ...
  const formatCurrency = (amount) => {
    return new Intl.NumberFormat('en-BD', { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(amount || 0);
  };
  ...
  const getMethodColor = (method) => {
    const map = { bkash: 'text-pink-400', nagad: 'text-orange-400', rocket: 'text-purple-400', upay: 'text-blue-400', bank: 'text-teal-400', card: 'text-indigo-400' };
  ...
  const exportToCSV = async () => {
    try {
      ...
      link.href = blobUrl;
      link.download = `approved_deposits_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const inputClass = 'w-full bg-[#0F111A] border border-gray-700 text-gray-200 text-xs rounded px-3 py-2 focus:outline-none focus:border-emerald-500 placeholder-gray-600';
  const selectClass = 'w-full bg-[#0F111A] border border-gray-700 text-gray-200 text-xs rounded px-3 py-2 focus:outline-none focus:border-emerald-500';
      ...
      <div className="flex pt-[10vh]">
        ...
        >
          ...
          <div className="rounded-lg mb-8 flex flex-col md:flex-row justify-between items-center">
            <div>
              <h1 className="text-2xl font-semibold text-white tracking-tighter uppercase">Approved Deposits</h1>
              <p className="text-xs font-bold text-gray-500 mt-1 flex items-center gap-2">
                <FaCalendarAlt className="text-emerald-500" /> View and manage approved deposit transactions
              </p>
            </div>
            <div className="flex gap-3 mt-4 md:mt-0">
              <button
                onClick={() => { fetchDeposits(); fetchStats(); }}
                className="bg-[#1F2937] hover:bg-emerald-600 border border-gray-700 px-6 py-2 rounded font-bold text-xs transition-all flex items-center gap-2"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            {[
              { label: 'TOTAL APPROVED', value: stats.total, color: 'border-emerald-500', valueClass: 'text-emerald-400' },
              { label: 'TOTAL AMOUNT', value: `৳${formatCurrency(stats.totalAmount)}`, color: 'border-indigo-500', valueClass: 'text-white' },
              { label: 'APPROVED AMOUNT', value: `৳${formatCurrency(stats.approvedAmount)}`, color: 'border-teal-500', valueClass: 'text-teal-400' },
            ].map((card, i) => (
          ...
          <div className="bg-[#161B22] border border-gray-800 rounded-lg p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-gray-400 flex items-center gap-2">
                <div className="w-1 h-4 bg-emerald-500"></div> Filters & Search
              </h2>
              <button
          ...
          <div className="mb-3 flex justify-between items-center">
            ...
            <p className="text-[10px] text-emerald-400 font-black uppercase tracking-wider">
              Approved: ৳{formatCurrency(stats.approvedAmount)}
            </p>
          </div>
                  ...
                  {loading ? (
                    ...
                    sortedDeposits.map((deposit) => (
                      <tr key={deposit._id} className="hover:bg-[#1F2937] transition-colors">
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <span className="text-[9px] px-2 py-1 rounded font-bold uppercase flex items-center gap-1 w-fit bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                    ...
                    <tr>
                      <td colSpan="8" className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center text-gray-600">
                          ...
                          <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">No approved deposits found</p>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs font-bold border transition-all ${
                    currentPage === 1
                      ...
                      : 'bg-[#1C2128] border-gray-700 text-gray-300 hover:bg-emerald-600 hover:border-emerald-500'
                  }`}
                >
                  ← Prev
                </button>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[1000] backdrop-blur-sm p-4">
          <div className="bg-[#161B22] border border-gray-700 rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-800 flex justify-between items-center bg-[#1C2128]">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-400">Approved Deposit Details</h3>
            ...
            <div className="px-6 py-5">
              ...
              <div className="mb-5 flex items-center gap-3">
                ...
                <span className="text-[9px] px-3 py-1 rounded font-bold uppercase flex items-center gap-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
      ...
      {showEditModal && selectedDeposit && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[10000] backdrop-blur-sm p-4">
          <div className="bg-[#161B22] border border-gray-700 rounded-lg shadow-2xl max-w-md w-full">
            <div className="px-6 py-4 border-b border-gray-800 flex justify-between items-center bg-[#1C2128]">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-400">Edit Approved Deposit</h3>
            ...
            <div className="px-6 py-5">
              ...
              >
                ...
                <div className="mt-6 flex justify-end gap-3">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-bold transition-all"
                  >
                    Save Changes
                  </button>
                </div>
